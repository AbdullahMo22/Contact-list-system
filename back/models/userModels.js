...
exports.updateUserScope = async (userId, hotelIds, departmentIds) => {
...
exports.getUserScope = async (userId) => {
    ...
    return {
        hotelIds: hotelRows.map(row => row.hotel_id),
        departmentIds: deptRows.map(row => row.department_id)
    };
};
