...
const Alert: React.FC<{
  ...
  return (
    <div className={`border rounded-lg px-4 py-3 flex items-start justify-between gap-4 ${styles}`}>
...
const RoleManagement: React.FC = () => {
        ...
        <div className="px-4 sm:px-0 space-y-6">
          ...
          <div className="bg-white shadow-sm border rounded-xl">
            <div className="px-4 py-5 sm:p-6">
              <div className="flex items-center justify-between gap-4 mb-4">
                <h2 className="text-lg font-semibold text-gray-900">قائمة الأدوار</h2>
                <button
                  onClick={fetchRoles}
                  className="text-sm px-3 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 transition"
                >
                  تحديث
                </button>
              </div>
              ...
              {roles.length === 0 ? (
                <div className="text-center py-10">
                  <p className="text-gray-500">لا توجد أدوار محددة</p>
                </div>
              ) : (
  ...
  className="px-3 py-1.5 rounded-lg text-sm text-gray-700 bg-gray-100 hover:bg-gray-200 transition"
>
  تعديل
</button>
                                ...
                                <button
                                  ...
                                  className={`px-3 py-1.5 rounded-lg text-sm transition ${deletable && deletingId !== role.id
                                      ? 'text-red-700 bg-red-50 hover:bg-red-100'
                                      : 'text-gray-400 bg-gray-50 cursor-not-allowed'
                                    }`}
                                >
                                  {deletingId === role.id ? 'جارٍ الحذف...' : 'حذف'}
                                </button>


                                <Link
                                  to={allRoutes.adminRolePermissions(role.id)}
                                  className="px-3 py-1.5 rounded-lg text-sm text-indigo-700 bg-indigo-50 hover:bg-indigo-100 transition"
                                >
                                  إدارة الصلاحيات
                                </Link>
                              </div>
